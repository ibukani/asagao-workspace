...
adapter は security policy、audit event semantics、Workspace lifecycle、MCP tool contract の source of truth にはなりません。service 層が policy / audit を通したうえで adapter を呼び出し、adapter 固有の error や metadata は Asagao 側の structured error へ変換します。
