...
| Issue | 適用方針 |
|---|---|
| #9 apply_patch | `git apply --numstat -z` / `git apply --check` / `git apply` を `GitAdapter` 経由にする。patch parser / applicator の自前再実装は避け、path safety、policy、audit、lifecycle は Asagao service 層で維持する |
| #10 git status / workspace diff | 既存実装を #36 で監査し、git invocation と parser を adapter 境界へ寄せる |
| #11 command job | `CommandJobService` から `ProcessRunner` と `JobQueue` を使い、tool handler が外部ライブラリを直接呼ばない。argument array、policy/audit、timeout、stdout/stderr、workspace concurrency を Asagao service 層で維持する |
