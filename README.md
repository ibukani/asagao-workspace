...
- Runner 操作向けの security boundary、workspace policy、command policy、audit event model、log masking 拡張点。
...
`src/security/` は、将来の file、patch、command、artifact 操作が共有する安全境界です。現在は実行ツール本体ではなく、次の基盤を提供します。
...
- audit event の共通形式と recorder interface。
- audit metadata や command log に適用できる log masking extension point。

repository clone、patch 適用、shell 実行、file read/write tool はまだ行いません。これらを追加する場合は、実際の副作用を起こす前に `src/security/` の policy を参照し、audit event を記録する必要があります。
...
1. Runner 安全ポリシーと監査ログの基盤を各操作 tool に接続する。
